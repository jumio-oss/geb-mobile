 ...
 */
@Slf4j
class AndroidInstrumentationNonEmptyNavigator extends AbstractNavigator {
    ...
    private boolean matchesSelector(RemoteWebElement element, String selectorString) {
        log.debug("SelectorString: $selectorString")
        if (selectorString.startsWith("#")) {
            return element.id == selectorString.substring(1)
        } else {
            def m = pat.matcher(selectorString)
            if (m.matches()) {
                def val = element."${m[0][1]}"
                def ret = val == m[0][2]
                log.debug "Matches: element.${m[0][1]}: $val == ${m[0][2]} : $ret"
                return ret
            }
        }
    }
